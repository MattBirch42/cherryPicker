...
server_data_import <- function(id, rvals, preloaded_data = NULL) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    message(
      "preloaded_data class: ",
      paste(class(preloaded_data), collapse = ", ")
    )
    ...
    observe({
      if (
        ...
        message(
          "Connection object assigned. Valid? ",
          DBI::dbIsValid(rvals$con.app)
        )
    ...
    observe({
      ...
      if (is.null(rvals$con.app) || !DBI::dbIsValid(rvals$con.app)) {
        return()
      }
      ...
      if (!is.null(preloaded_data) && is.null(isolate(rvals$data))) {
        message(
          "Preloading dataset into DuckDB (delayed until connection ready)..."
        )
        ...
        showNotification(
          paste0(
            "Preloaded dataset loaded into DuckDB (",
            ncol(preloaded_data),
            " columns, ",
            nrow(preloaded_data),
            " rows)."
    ...
    observeEvent(input$file, {
      ...
      df <- switch(
        ...
        {
          showNotification(
            "Unsupported file type. Please upload CSV or Parquet.",
            type = "error"
          )
          return(NULL)
        }
      ...
      showNotification(
        paste0(
          "Uploaded dataset loaded into DuckDB (",
          ncol(df),
          " columns, ",
          nrow(df),
          " rows)."
        ...
        tags$table(
          ...
          tags$thead(
            tags$tr(
              tags$th("Variable"),
              tags$th("Type"),
              tags$th("Unique Values"),
              tags$th("Range (if applicable)"),
              tags$th("NAs"),
              tags$th("Not NAs")
            )
          ),
