  ...
  function(input, output, session) {
    ...
    shiny::observeEvent(input$file, {
      if (is.null(preloaded_data)) {
        ...
        df <- detect_and_convert_characters(
          df,
          session,
...
"
    ...
    shiny::observeEvent(input$export_to_r, {
      ...
      shiny::observeEvent(
        input$confirm_export_to_r,
        {
          ...
          cherry_picker_export_list <- get(
            "cherry_picker_export_list",
            envir = .GlobalEnv
          )
          cherry_picker_export_list[[
            length(cherry_picker_export_list) + 1
          ]] <- export_entry
          assign(
            "cherry_picker_export_list",
            cherry_picker_export_list,
    ...
    shiny::observeEvent(input$export_to_r_remove, {
      ...
      # Handle confirm button
      shiny::observeEvent(
        input$confirm_export_to_r_remove,
        {
          shiny::removeModal()
          ...
          shiny::showNotification(
            "Export added to R session and selections removed from data",
            type = "message"
          )
        },
        ignoreInit = TRUE,
