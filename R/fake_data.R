...
fake_data <- function(N = 20000, seed = 42) {
  ...
  df <- tibble(
    ...
    weekday       = as.character(wday(start_time + hours(0:(N - 1)), label = TRUE, abbr = TRUE))
  )
  ...
  df <- df %>%
    mutate(
      temp_avg = pmax(0, pmin(100, season_effect + daily_effect + rnorm(N, sd = 5))),
      linear   = 42 + 0.02 * (1:N) + rnorm(N, 0, 2),
      smile    = 100 - 150 * (seq(-1, 1, length.out = N)^2) + rnorm(N, 0, 5),
      noise    = runif(N, 0, 80),
      noise2   = (noise - 20)^2,
      little_string = sample(c("oranges", "cat", "economics", "widget"), N, TRUE),
      big_string    = sample(apply(expand.grid(LETTERS, LETTERS), 1, paste0, collapse = ""), N, TRUE),
      daily_thing   = 5 * as.numeric(hour_begin) + rnorm(N, 0, 3)
    )
  
  # --- Inject missing values (~1/1000 rows) ---
  rows_na  <- sample(seq_len(N), N / 1000)
  cols_na  <- sample(setdiff(names(df), c("timestamp_utc", "date", "weekday")), length(rows_na), replace = TRUE)
  ...
  cols_out <- sample(setdiff(names(df), c("timestamp_utc", "date", "weekday")), length(rows_out), replace = TRUE)
  for (i in seq_along(rows_out)) {
    ...
    if (is.numeric(val) && !is.integer(val)) {
      ...
      df[[col]][rows_out[i]] <- paste0("OUTLIER_", paste0(sample(letters, 6, TRUE), collapse = ""))
    }
  }
