...
make_marginal_scatter <- function(df,
  ...
  if (is.character(plot_df[[xvar]])) plot_df[[xvar]] <- as.factor(plot_df[[xvar]])
  if (is.character(plot_df[[yvar]])) plot_df[[yvar]] <- as.factor(plot_df[[yvar]])
  ...
  if (x_is_num) {
    ...
    xhist <- plot_df %>%
      ...
      dplyr::group_by(bin) %>%
      dplyr::summarise(count = dplyr::n(), ids = list(.row_uid), center = mean(.data[[xvar]], na.rm = TRUE), .groups = "drop")
  ...
  if (y_is_num) {
    ...
    yhist <- plot_df %>%
      ...
      dplyr::group_by(bin) %>%
      dplyr::summarise(count = dplyr::n(), ids = list(.row_uid), center = mean(.data[[yvar]], na.rm = TRUE), .groups = "drop")
  ...
  p <- p %>% plotly::event_register("plotly_click") %>% plotly::event_register("plotly_selected")
  p
}
